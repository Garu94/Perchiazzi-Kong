...
//                checkUltimateDeath()
  ...
  func checkDespawnBarrel() {
    ...
    func checkMaxScore() {
        let topScore = UserDefaults.standard.string(forKey: "topScore")
        if GameManager.shared.score > Int(topScore!)! {
            UserDefaults.standard.set(GameManager.shared.score, forKey: "topScore")
        }
    }
    
    func win() {
        GameManager.shared.score += GameManager.shared.bonus
